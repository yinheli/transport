...
@click.option("-e", "--encoding", default='UTF-8', help="hql 文件编码，默认为 UTF-8")
@click.option("-o", "--out", default=os.getcwd(), type=click.Path(exists=True), help="输出文件夹，默认输出到当前文件夹")
def main(excel: str, sql: str, out: str, encoding: str):
    """一个简单批处理工具，输入整理好的 excel 和 hql 表结构，基于模板输出 ddl 和 procedure"""
    transport = Transport(excel, sql, out, encoding=encoding)
    transport.handle()
